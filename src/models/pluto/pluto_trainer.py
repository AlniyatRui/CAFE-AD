...
class LightningTrainer(pl.LightningModule):
    ...
    def _step(
        ...
        res = self.forward(features["feature"].data, warm_up)
        
        original_target = features["feature"].data["agent"]["target"].clone()
        original_valid_mask = features["feature"].data["agent"]["valid_mask"].clone()
        original_velocity = features["feature"].data["agent"]["velocity"].clone()
        
        ## for prune trainging ##
        features["feature"].data["agent"]["target"] = res["neighbors_future"]
        features["feature"].data["agent"]["valid_mask"] = res["agent_valid_mask"]
        features["feature"].data["agent"]["velocity"] = res["neighbors_vel"]
        ## for prune trainging ##

        devices = res["prediction"].device

        losses = self._compute_objectives(res, features["feature"].data)
        metrics = self._compute_metrics(res, features["feature"].data, prefix)
        
        features["feature"].data["agent"]["target"] = original_target
        features["feature"].data["agent"]["valid_mask"] = original_valid_mask
        features["feature"].data["agent"]["velocity"] = original_velocity
        
        self._log_step(losses["loss"], losses, metrics, prefix)

        model_loss = losses["loss"] if self.training else 0.0

        scenario_type = [scenario._scenario_type for scenario in scenarios]
        scenario_type_target = torch.tensor([scenario_label_dict[i] for i in scenario_type], device=devices)
        
        if self.training and self.use_collision_loss:
            scenario_type_target = scenario_type_target.repeat(3)
        
        return model_loss, scenario_type_target, devices

    def _step_augment(self, batch, scenario_type_target, scenario_gradient):
        features, targets, scenarios = batch
        ...
        res = self.model.train_f(features["feature"].data, scenario_type_target, scenario_gradient, warm_up)
    ...
    def training_step(
        ...
        model_loss, scenario_type_target, devices = self._step(batch, "train")    

        if self.current_epoch < self.warmup_epochs:
            # print(type(self.optimizers()[0]))
            ...
            # if self.current_epoch == self.warmup_epochs:
            #     print('start mixup')
            self.scenario_gradient = None 
            feature = self.model.get_feature(features["feature"].data, warm_up).detach()
            feature.requires_grad_(True)
            result = self.model.domain_classifier(feature)
            
            loss_domain = F.cross_entropy(result, scenario_type_target)
            
            self.optimizers()[1].zero_grad()
            self.manual_backward(loss_domain, retain_graph=True)
            self.scenario_gradient = feature.grad
            
            loss1 = model_loss
            loss2 = self._step_augment(batch, scenario_type_target, self.scenario_gradient) 
            
            loss_class = 0.5*loss1+0.5*loss2
            self.optimizers()[0].zero_grad()
            self.manual_backward(loss_class)
            self.optimizers()[0].step()
        ...
        loss_domain = F.cross_entropy(result, scenario_type_target)
        self.optimizers()[1].zero_grad()
        self.manual_backward(loss_domain)
        self.optimizers()[1].step()
            
        return {'loss': model_loss, 'domain_loss': loss_domain}
    ...
    def forward(self, features: FeaturesType, warm_up=False) -> TargetsType:
        """
        ...
        """
        return self.model(features, warm_up)
    ...
    def configure_optimizers(
        ...
        domain_params = {n: p for n, p in self.model.domain_classifier.named_parameters()}
        domain_param_names = set(domain_params.keys()) 
        ...
        for module_name, module in self.named_modules():
            for param_name, param in module.named_parameters():
                ...
                if full_param_name in domain_param_names:
                    # import pdb; pdb.set_trace()
                    continue
                ...
                if "bias" in param_name:
                    ...
                    no_decay.add(full_param_name)
        param_dict = {
            param_name: param for param_name, param in self.named_parameters() if param_name not in domain_param_names
        }
        ...
        optimizer_domain = torch.optim.AdamW(
            self.model.domain_classifier.parameters(), lr=self.lr, weight_decay=self.weight_decay
        )
        ...
        return [optimizer, optimizer_domain], [scheduler, scheduler_domain]
